...
const Signup = () => {
  ...
  const [role, setRole] = useState<"undergrad" | "postgrad">("undergrad");
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Button type="button" variant={role === "undergrad" ? "default" : "outline"} size="sm" onClick={() => setRole("undergrad")} className="flex-1">Undergraduate</Button>
              <Button type="button" variant={role === "postgrad" ? "default" : "outline"} size="sm" onClick={() => setRole("postgrad")} className="flex-1">Postgraduate</Button>
            </div>
          </div>
          <div className="flex items-start gap-2">
            ...
            <Label htmlFor="terms" className="text-sm font-normal text-muted-foreground leading-tight">
              I agree to the <Link to="/terms" className="underline text-foreground">Terms</Link> and <Link to="/privacy" className="underline text-foreground">Privacy Policy</Link>
            </Label>
          </div>
          <Button className="w-full" size="lg" disabled={!agreed} asChild>
